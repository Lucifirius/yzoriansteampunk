...
namespace {
  ...
  bool canBeHarvestedBy(const CUnit* resource, u8 playerId) {
    ...
    if (resource != NULL
      ...
      if (UnitId::ResourceMineralField <= resource->id
		  ...
		  //scbw::printText("rallied to: mineral field");
        return true;
      ...
      if (resource->status & UnitStatus::Completed
		  ...
		  //scbw::printText("rallied to: geyser");
        return true;
    }
	
	//scbw::printText("rallied to: no resource");
    return false;
  }
  ...
  void manageRally(CUnit *unit, CUnit *factory) {
    using units_dat::BaseProperty;
    //Do nothing if the rally target is the factory itself, the factory doesn't exist, or the rally target position is 0
	if (factory == NULL) return; //this makes units from eggs have no rally point if their parent hatchery was destroyed before they hatch
    ...
    if (BaseProperty[unit->id] & UnitProperty::Worker
		...
		//scbw::printText("ordered to: harvest");
        return;
    }
    ...
    if (factory->rally.unit && scbw::canBeEnteredBy(factory->rally.unit, unit)) {
      unit->orderTo(OrderId::EnterTransport, factory->rally.unit);
		//scbw::printText("ordered to: enter transport");
      return;
    }
